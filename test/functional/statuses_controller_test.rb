...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should display user's post when not logged in" do
    ...
    users(:michael).statuses.create(content: 'Non-blocked status')
    get :index
  ...
  test "should be logged in to post a status" do
    post :create, status: { content: "Hello" } # posting to the create method with the status of hello
    assert_response :redirect
    assert_redirected_to new_user_session_path
  end
  ...
  test "should create status for the current user when logged in" do
    ...
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:sammy).id
  end
